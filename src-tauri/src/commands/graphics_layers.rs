...
use crate::commands::runner_downloads::{normalize_extracted_dir, snapshot_entries};
...
async fn ensure_layer(
    ...
    let before = snapshot_entries(&cache);
    extract_archive(&asset_name, &bytes, &cache)?;
    normalize_extracted_dir(&cache, &before, dir_name)?;
...
pub async fn ensure_wine_mono_msi(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    let index = reqwest::Client::new()
        .get("https://dl.winehq.org/wine/wine-mono/")
        .header(USER_AGENT, "prefixr")
        .send()
        .await
        .map_err(|e| format!("Could not reach dl.winehq.org: {e}"))?
        .text()
        .await
    ...
    let version_index = reqwest::Client::new()
        .get(&version_url)
        .header(USER_AGENT, "prefixr")
        .send()
        .await
    ...
    let bytes = reqwest::Client::new()
        .get(format!("{version_url}{msi_name}"))
        .header(USER_AGENT, "prefixr")
        .send()
        .await
    ...
    fs::write(&msi_path, &bytes).map_err(|e| format!("Could not save {msi_name}: {e}"))?;
    Ok(msi_path)
}
